<script setup>
    ...
    const copy = () => {
        navigator.clipboard.writeText(location.origin + '?ref=' + user.active_ref_link.id)
            .then(() => {
                copyShow.value = true;
                setTimeout(() => {
        ...
        <div class="main__ref">
            ...
            <div v-if="user.ref_link_count > 0">
                ...
                <div v-else class="mb-20">
                    <div class="font-bold">{{ $t('referral.percent') }}:</div>
                    <div class="flex mt-2 mb-5 justify-evenly gap-x-2">
                        <div>
                            <div class="text-sm">UMCT</div>
                            <div class="text-center" v-text="user.active_ref_link.umt_percent + '%'"></div>
                        </div>
                        <div>
                            <div class="text-sm">USDT</div>
                            <div class="text-center" v-text="user.active_ref_link.usdt_percent + '%'"></div>
                        </div>
                    </div>
                    <div class="mb-1 font-bold">{{ $t('referral.link') }}:</div>
                    <div class="relative px-2 py-3 mb-8 text-center border rounded border-pink" @pointerdown="copy">
